...
def main():
    ...
    #print ('max',max_article_length)
    test_sequences = pad_sequences(test_sequences,maxlen=max_article_length)
    
    pickle.dump(tokenizer,open('rnntk','wb'))
    print ('alreadysave')
    train_tag = to_multi_categorical(Y_data,tag_list) 
    #print (train_tag)
    tt = np.zeros((38,1))
    for x in range(train_tag.shape[0]):
        for y in range(38):
            if train_tag[x][y] == 1:
                tt[y]+=1
    for x in range(38):
        print(tag_list[x],tt[x])
    ...
    model.add(Embedding(num_words,
                        ...
                        trainable=False))
    model.add(GRU(128,activation='tanh',dropout=0.25,return_sequences=True))

...
def MF(umax,mmax,dim,lmbda):
	x_u = Input(shape=(1,))
	x_m = Input(shape=(1,))
	embedding_u = Embedding(umax+1,dim,embeddings_regularizer= l2(lmbda))(x_u)
	embedding_m = Embedding(mmax+1,dim,embeddings_regularizer= l2(lmbda))(x_m)
	f_u = Flatten()(embedding_u)
	f_m = Flatten()(embedding_m)
	f_u = Dropout(0.4)(f_u)
	f_m = Dropout(0.4)(f_m)
	p = dot([f_u,f_m],axes=1)
	u_bias = Embedding(umax+1,1,embeddings_initializer='zeros')(x_u)
	m_bias = Embedding(mmax+1,1,embeddings_initializer='zeros')(x_m)
	u_bias = Flatten()(u_bias)
	m_bias = Flatten()(m_bias)
	p = add([p,u_bias,m_bias])
	p=Dense(1)(p)
...
checkpoint = ModelCheckpoint(filepath='best.hdf5',
                             ...
                             monitor='val_loss',
                             mode='min')
model.fit([user,movie],rank,epochs=15, batch_size=1024)
...
print ('moviie_emb shape',movie_emb.shape)
